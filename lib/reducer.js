  ...
  if (action === 'undo') {
    ...
    if (action === 'down') {
      ...
      if (action === 'drop' && state.lines.slice(from, to + 1).some(line=>line.action === 'break')) {
        const newLines = newState.lines.filter((line, idx)=> idx < from || idx > to || line.action !== 'break');
        const removedLines = newState.lines.length - newLines.length;
        newState = {
          lines: newLines,
          cursor: {
            from: state.cursor.from < state.cursor.pos ? state.cursor.from : state.cursor.from - removedLines,
            pos: state.cursor.pos < state.cursor.from ? state.cursor.pos : state.cursor.pos - removedLines,
          }
        }
      }
      if (newState !== state) {
