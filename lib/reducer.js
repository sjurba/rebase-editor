...
function insertAfterCurrentPosition(state, content) {
    let newLines = state.lines;
    let line = {action : 'break', hash : '', message : 'break'}
    insertInto(newLines, line,  state.cursor.pos + 1);
    return {
        lines: newLines,
        cursor: {
            from: state.cursor.from + 1,
...
module.exports = function reducer(state, action, param) {
  ...
  if (action === 'undo') {
    ...
    if (action === 'down') {
      ...
      state = set(state, insertAfterCurrentPosition(state));
    }
