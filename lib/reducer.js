...
function getCursor(state, pos, select) {
  return {
    cursor: {
      pos: pos,
      from: select ? state.cursor.from : pos
    }
  };
}

function getCursorByDelta(state, change, select) {
  return getCursor(state, state.cursor.pos + change, select);
}
  ...
  if (action === 'undo') {
    ...
    if (action === 'down') {
      if (pos !== state.lines.length - 1) {
        state = set(state, getCursorByDelta(state, DOWN));
      }
    } else if (action === 'up') {
      if (pos !== 0) {
        state = set(state, getCursorByDelta(state, UP));
      }
    } else if (action === 'end') {
      if (pos !== state.lines.length - 1) {
        state = set(state, getCursor(state, state.lines.length - 1));
      }
    } else if (action === 'home') {
      if (pos !== 0) {
        state = set(state, getCursor(state, 0));
      }
    } else if (action === 'selectDown') {
      if (pos !== state.lines.length - 1) {
        state = set(state, getCursorByDelta(state, DOWN, true));
      }
    } else if (action === 'selectUp') {
      if (pos !== 0) {
        state = set(state, getCursorByDelta(state, UP, true));
      }
    } else if (action === 'selectHome') {
      if (pos !== 0) {
        state = set(state, getCursor(state, 0, true));
      }
    } else if (action === 'selectEnd') {
      if (pos !== state.lines.length - 1) {
        state = set(state, getCursor(state, state.lines.length - 1, true));
      }
