...
module.exports = class Terminal {
  ...
  render(state, key, rawKey) {
    ...
    const currentLines = [];
    if (this.opts.status) {
      currentLines.push(`Cursor: ${state.cursor.pos} From: ${state.cursor.from} Key: ${key}  Raw key: ${rawKey} Height: ${this.term.height}`);
    }
    ...
    let offset = 0;
    if (state.cursor.pos >= this.term.height) {
      offset = state.cursor.pos - this.term.height + 1;
    }
    currentLines.slice(offset, this.term.height + offset).forEach((line, index) => {
      this._writeLine(line, index);
    });
    this._moveTo(state.cursor.pos + (this.opts.status ? 1 : 0));
  }
};
