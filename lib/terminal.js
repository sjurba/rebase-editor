...
module.exports = class Terminal {
  ...
  _writeLine(line) {
    ...
    console.log(index, offset, this.lastOffset, this.currentLines[index - offset + this.lastOffset] !== line, line, this.currentLines[index - offset + this.lastOffset]);
    if (index > offset && this.currentLines[index - offset + this.lastOffset] !== line && index - offset <= this.term.height) {
      this.term.moveTo(1, index - offset);
      this.term.eraseLine();
      this.term(line);
    }
  ...
  render(state, key, rawKey) {
    ...
    if (this.opts.status) {
      this._writeLine(`Cursor: ${state.cursor.pos} From: ${state.cursor.from} Key: ${key}  Raw key: ${rawKey} Height: ${this.term.height}`);
    }
