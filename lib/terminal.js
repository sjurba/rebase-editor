...
module.exports = class Terminal {
  ...
  render(oldState, state, key, rawKey) {
    ...
    let startIndex = 1;
    if (this.opts.status) {
      this._writeLine(1, false, `Cursor: ${state.cursor.pos}  From: ${state.cursor.from}  Key: ${key}  Raw key: ${rawKey}`);
      startIndex++;
    }
    let index = startIndex;
    state.lines.forEach((line, idx) => {
      const selected = inSelection(state, idx);
      if (!oldState || oldState.lines[idx] !== line || selected !== inSelection(oldState, idx)) {
        if (this.opts.colors) {
        ...
        if (line === '#' && state.extraInfo) {
          ...
          if (emptyLines === 2) {
            state.extraInfo.forEach((line) => {
              this._writeLine(index, false, line);
              index++;
            });
          }
        }
